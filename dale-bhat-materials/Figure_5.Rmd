---
title: 'Figure 5: Reconstruction the Two-Well Attractor of Tuller et al. (1994)'
author: "Rick Dale"
date: "5/31/2018"
...
for (i in 1:100) { # 100 trials of the run
  ...
  for (j in 1:1000) { # run a while until it hits threshold (almost always much shorter of course)
    ...
    x = x + (-k + x - x^3) # update state variable
    x_sum = x_sum + x
...
sindy.obj = sindy(xs=deets[1:(nrow(deets)-1),1:2],dx=as.matrix(deets[2:nrow(deets),1:2]),lambda=.7,plot.eq.graph=T);
print(sindy.obj$B)

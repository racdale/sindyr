---
title: 'Figure 6: The Effect of Noise on Two-Attractor Well Reconstruction'
...
Taking the simulation of two-attractor well decision making described in Figure 5 and in Dale and Bhat, we inject noise and demonstrate that SINDy becomes highly unstable. 
...
logistic_sindy = function(trials=100,noise=0,more.complex=F) {
  ...
  for (i in 1:trials) { # let it go for 100 trials
    ...
    for (j in 1:1000) { # let it take at most 1000 cycles to settle
      x_trial = rbind(x_trial,data.frame(k=k,x=x,x_sum=x_sum)) # store this trial
      if (more.complex) {
  ...
  sindy.obj = sindy(xs=deets[1:(nrow(deets)-1),1:2],dx=as.matrix(deets[2:nrow(deets),1:2]),lambda=.6,B.expected=B.expected)
  return(sindy.obj)
}
...
for (noise in seq(from=0,to=.25,length=100)) { # noise level varies
...
complexities = c()
noise = .2 # set a high noise level, judging from the above simulation

for (iter in 1:100) {

  set.seed(666) # seed start the same for each condition

...
for (lambda in seq(from=.001,to=.07,by=.005)) {
for (pair in pairs) {
for (trial in trials) { 
for (task in tasks) {
      ...
      if (nrow(data)>3600) {
        ...
        # TEST
        xs = data[ceil(nrow(data)/2):nrow(data),]
        Theta = features(data.frame(xs),2)
        dx = finDiffs(xs,1)
...
save(file='output/fits_halves.Rd',fits)
save(file='output/coefs_halves.Rd',coefs)

load('output/fits_halves.Rd')

pdf(file='output/data_aggregate.pdf',height=6,width=11)
par(mfrow=c(1,2))
l_maxes = c()
i=0
deets = coefs[[1]][[1]]*0
